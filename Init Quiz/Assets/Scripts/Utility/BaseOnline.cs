...
public class BaseOnline : MonoSingleton<BaseOnline>
{
    ...
    public static float DefaultTimeOut = 60;
...
#if DEV
            if (IsSuccess(www.text) == false)
	        {
                string _log = www.text;
                if (_log.Length > 5000) _log = _log.Substring(0, 5000);
                PopupManager.ShowYesNoPopUp(www.url, _log, () => {ShareImage.Instance.onShareSimpleText(_log);}, null, "SHARE", "CLOSE");
            }
#endif
        }
